#
# PowerShell Module: KeepComputerAwake
...
if ($PSVersionTable.PSVersion.Major -lt 5 -or (-not $IsWindows -and $PSVersionTable.PSVersion.Major -ge 6)) {
    throw "This module requires Windows operating system."
}
...
# Hide the PowerShell console window helper
...
$script:keepDisplayOn = $true
$script:startTime = $null
$script:endTime = $null
$script:notifyIcon = $null
$script:timer = $null
$script:timeMinutesParam = 0

# Internal function to set the awake state
...
# Internal function to reset the awake state (return to normal power settings)
function Reset-AwakeState {
    ...
    try {
        ...
        Write-Verbose "Awake state reset. System returning to normal power settings."
    }
    catch {
...
# Internal function to get formatted time remaining
...
# Internal function to check if time has expired
...
# Internal function to create and manage system tray icon
function Start-SystemTray {
    ...
    $timeStatus = if ($null -eq $script:endTime) { "Indefinite" } else { "$($script:timeMinutesParam) minutes" }
...
.DESCRIPTION
    This function prevents the computer and display from sleeping by using the same
...
.PARAMETER HideConsole
    If specified, hides the PowerShell console window. Only effective when using system tray icon.
...
.EXAMPLE
    ...
    Keeps computer awake with hidden console window.

.NOTES
    Based on PowerToys Awake implementation
    Requires: PowerShell 5.1 or higher, Windows OS
#>
function Start-ComputerAwake {
    [CmdletBinding()]
    param(
    ...
    $script:endTime = $null
    $script:timeMinutesParam = $TimeMinutes

    if ($TimeMinutes) {
        $script:endTime = $script:startTime.AddMinutes($TimeMinutes)
    }

    # Hide the console window if requested and not in NoTray mode
    if ($HideConsole -and -not $NoTray) {
    ...
    try {
        ...
        Write-Host "  Keep Computer Awake Module" -ForegroundColor Cyan
        ...
        if ($null -ne $script:endTime) {
            ...
            Write-Host "Duration: Indefinite (until stopped)" -ForegroundColor Green
        }
        ...
        try {
            Write-Host "Module function terminated. Computer returning to normal power settings." -ForegroundColor Green
        } catch {
            # Ignore if pipeline is stopped
        }
    }
...
.DESCRIPTION
    This function resets the computer to its normal power settings by clearing
    the execution state flags set by Start-ComputerAwake.

.EXAMPLE
    Stop-ComputerAwake
    Returns computer to normal power settings.

.NOTES
    This function can be called independently to reset power settings.
#>
function Stop-ComputerAwake {
    [CmdletBinding()]
    param()

    try {
        Reset-AwakeState
        Write-Host "Computer awake state stopped. Returning to normal power settings." -ForegroundColor Green
    }
    catch {
        Write-Error "Failed to stop computer awake state: $_"
    }
}

# Export module functions
Export-ModuleMember -Function Start-ComputerAwake, Stop-ComputerAwake
